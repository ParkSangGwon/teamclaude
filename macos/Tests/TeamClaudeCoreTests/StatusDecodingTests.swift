...
final class StatusDecodingTests: XCTestCase {
    ...
    func testHostileFixtureDegradesInsteadOfThrowing() throws {
        ...
        XCTAssertEqual(a.name.count, 250, "an identifier is stripped of control characters but never shortened: a shortened name matches nothing")
        XCTAssertTrue(a.name.allSatisfy { $0 == "x" })
    ...
    func testTextSafe() {
        ...
        XCTAssertEqual(Text.safe(String(repeating: "x", count: 512)).count, 512)
        let capped = Text.safe(String(repeating: "x", count: 513))
